...
_OFFER_TOKEN = re.compile(r"\boffer(?:ed|ing|s)?\b", re.I)


def _offer_requires_review(subject: str, body: str) -> bool:
    return bool(_OFFER_TOKEN.search(f"{subject}\n{body}"))
...
class ComposioClient:
    def __init__(self, account: str):
        if not isinstance(account, str) or not account.strip():
            raise ValueError("Composio account alias must be non-empty")
        ...
        try:
            ...
            if isinstance(failure, ComposioUnavailableError):
                raise ComposioUnavailableError(
                    f"Composio Gmail connection is unavailable for configured account alias {self.account!r}"
                ) from None
            raise ComposioError("Composio command failed") from None
        except FileNotFoundError:
            raise ComposioUnavailableError(
                f"Composio CLI is unavailable for configured account alias {self.account!r}"
            ) from None
        except OSError:
            raise ComposioError("Composio command failed") from None
...
def verify_mailbox(
    ...
    *,
    ...
    if getattr(client, "account", None) != expected_account_alias:
        raise ComposioError("Composio account alias differs from local analytics config")
    runner = execute or (
...
def classify_message(message: Mapping[str, object]) -> MailSignal | None:
    ...
    classification_subject = _sanitize_text(message.get("subject"))
    subject = _sanitize_text(message.get("subject"), 200)
    searchable = f"{classification_subject} {body}".strip()

    event_type: str | None = None
    requires_manual_review = _offer_requires_review(classification_subject, body)
    if requires_manual_review:
        event_type = "offer"
    elif any(pattern.search(searchable) for pattern in _REJECTION_PATTERNS):
        event_type = "rejected"
    ...
    if event_type == "rejected" and _LOGISTICS.search(body):
        ...
        signal = "The employer extended an explicit job offer."
        required_action = "Review the offer terms and response deadline."
...
def _canonical_company(
    value: object, company_aliases: Mapping[str, str] | None = None
) -> str:
    ...
    reduced = " ".join(words)
    return aliases.get(reduced, reduced)
...
def _recognized_sender(
    sender: str, company: str, company_aliases: Mapping[str, str] | None = None
) -> bool:
    sender_tokens = set(re.findall(r"[a-z0-9]+", sender.casefold()))
    company_tokens = set(_canonical_company(company, company_aliases).split())
...
def match_application(
    ...
    signal_company = _canonical_company(signal.company, company_aliases)
    ...
    for application in applications:
        ...
        if not isinstance(application_id, str) or not application_id:
            continue
        if not signal_company or _canonical_company(
            application.get("company", ""), company_aliases
        ) != signal_company:
            continue
        ...
        if chronological:
            score += 0.10
        if _recognized_sender(signal.sender, signal.company, company_aliases):
            score += 0.05
...
def _needs_feedback_body(message: Mapping[str, object]) -> bool:
    if _message_body(message):
        return False
    subject = _sanitize_text(message.get("subject"))
    return _offer_requires_review(subject, "") or any(
        pattern.search(subject) for pattern in _REJECTION_PATTERNS
    )
...
def _scan_queries(
    ...
    after = (
        f"after:{_scan_start(applications, checkpoint):%Y/%m/%d} "
        "-in:sent -in:drafts"
    )
    ...
    *,
    ...
    for message in messages_for_classification:
        ...
        if signal is None or signal.source_ref in seen_source_refs:
            continue
        match = match_application(signal, applications, company_aliases)
        if signal.requires_manual_review:
            candidates = match.candidates
            if not candidates and match.application_id is not None:
                candidates = (match.application_id,)
            review_items.append(_review_item(
                ...
                MatchResult(
                    ...
                    candidates,
                    match.score,
                    "offer_requires_manual_confirmation",
                ),
            ))
            continue
