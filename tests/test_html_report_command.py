...
class HtmlReportTrackerFieldTests(unittest.TestCase):
    def test_command_routes_to_the_reviewed_dashboard_builder(self):
        text = COMMAND_FILE.read_text(encoding="utf-8")
        self.assertIn("python3 -m dashboard.build", text)
        self.assertIn("dashboard/index.html", text)

    def test_command_forbids_a_duplicate_tracker_parser(self):
        text = COMMAND_FILE.read_text(encoding="utf-8")
        self.assertIn("Do not reimplement CSV parsing", text)
        self.assertIn("24-column", text)
...
class HtmlReportGitignoreTests(unittest.TestCase):
    ...
    def test_dashboard_uses_lifecycle_history_and_outcome_semantics(self):
        text = COMMAND_FILE.read_text(encoding="utf-8")
        self.assertIn("analytics/application_events.csv", text)
        self.assertIn("lifecycle history", text)
        self.assertIn("candidate-initiated outcomes", text)
