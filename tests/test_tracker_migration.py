...
class TrackerMigrationTests(unittest.TestCase):
    ...
    def test_migration_reads_legacy_deadline_and_writes_normalized_field(self):
        with_deadline = read_csv_rows(
            FIXTURES / "legacy_tracker_deadline.csv", {"date", "deadline"}
        )[0]

        migrated = migrate_rows([with_deadline])

        self.assertEqual(migrated[0]["deadline"], "2026-09-30")

    def test_migration_adds_empty_deadline_to_pre_deadline_normalized_rows(self):
        legacy = read_csv_rows(FIXTURES / "legacy_tracker.csv", {"date"})
        normalized_without_deadline = migrate_rows(legacy)[0]
        normalized_without_deadline.pop("deadline")

        migrated = migrate_rows([normalized_without_deadline])

        self.assertEqual(migrated[0]["deadline"], "")


    def test_migration_preserves_final_status_vocabulary_as_closed(self):
        legacy = read_csv_rows(FIXTURES / "legacy_tracker.csv", {"date"})
        for status in (
            "no_response",
            "no response",
            ...
            with self.subTest(status=status):
                migrated = migrate_rows([{**legacy[0], "status": status}])[0]
                self.assertEqual(migrated["stage"], "closed")
                if " " in status:
                    self.assertEqual(migrated["status"], status.replace(" ", "_"))
