...
class UpskillSkillSpec(unittest.TestCase):
    ...
    def test_step2_column_list_matches_the_canonical_tracker_header(self):
        sections = _sections(SKILL.read_text(encoding="utf-8"))
        step2 = sections.get("Step 2: Load Data", "")
        self.assertIn(",".join(TRACKER_COLUMNS), step2)
    ...
    def test_step3_documents_dedupe_and_gap_precedence(self):
        ...
        self.assertIn("(100 - fit_score) / 100", step3)
        self.assertIn("(100 - rank_score) / 100", step3)

    def test_step3_handles_blank_fit_score(self):
        sections = _sections(SKILL.read_text(encoding="utf-8"))
        step3 = sections.get("Step 3: Pass 1 — Hard Skill Diff", "")
        self.assertIn(
            "blank or non-numeric `fit_score`",
            step3,
            "Step 3.3 must state what happens to a row whose fit_score is blank",
        )
        self.assertIn(
            "Never treat a blank as 0",
            step3,
            "blank fit scores must not receive maximum gap weight",
        )
