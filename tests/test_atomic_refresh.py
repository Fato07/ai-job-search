...
TEST_MAILBOX = "mailbox@example.test"
TEST_ACCOUNT_ALIAS = "candidate-mail"


class RecordingMailboxClient:
    account = TEST_ACCOUNT_ALIAS
    ...
    def execute(self, slug, data):
        ...
        if slug == "GMAIL_GET_PROFILE":
            return {"data": {"emailAddress": TEST_MAILBOX}}
...
class AtomicRefreshTests(unittest.TestCase):
    def _paths(self, root):
        ...
        (paths.root / "analytics" / "config.json").write_text(
            json.dumps({
                ...
                "gmail_account_alias": TEST_ACCOUNT_ALIAS,
                "gmail_expected_address": TEST_MAILBOX,
    ...
    def test_gmail_refresh_requires_local_account_configuration(self):
        with tempfile.TemporaryDirectory() as tmp:
            paths = self._paths(Path(tmp))
            (paths.root / "analytics" / "config.json").unlink()
            client = RecordingMailboxClient([])

            with self.assertRaisesRegex(
                ValueError,
                r"Copy analytics/config\.example\.json to analytics/config\.json",
            ):
    ...
    def test_offer_like_mail_queues_review_without_lifecycle_mutation(self):
        with tempfile.TemporaryDirectory() as tmp:
            paths = self._paths(Path(tmp))
            client = RecordingMailboxClient([
                self._message(
                    "offer-id",
                    body=(
                        "We offer reasonable accommodations. "
            ...
            self.assertEqual((application["stage"], application["status"]), ("prospect", "PROSPECT"))
            self.assertNotIn("offer", {event["event_type"] for event in events})
            self.assertEqual(summary.tracker_updates, 0)
            self.assertEqual(len(review), 1)
            self.assertEqual(review[0]["reason"], "offer_requires_manual_confirmation")
            self.assertEqual(json.loads(review[0]["candidate_application_ids"]), ["app-1"])
    def test_long_subject_offer_token_fails_closed_before_lower_priority_signal(self):
        with tempfile.TemporaryDirectory() as tmp:
            paths = self._paths(Path(tmp))
            before = {
                path: path.read_bytes()
                for path in (paths.tracker, paths.events, paths.feedback)
            }
            summary = refresh(
                paths,
                client=RecordingMailboxClient([
    ...
    def test_non_job_offer_tokens_also_queue_without_mutation(self):
        cases = (
            ...
            "We are offering support during the process.",
        )
        for index, body in enumerate(cases):
            with self.subTest(body=body), tempfile.TemporaryDirectory() as tmp:
                ...
                refresh(
                    paths,
                    client=RecordingMailboxClient([
                        self._message(f"non-job-offer-{index}", body=body)
                    ]),
                    sync_gmail=True,
                    now=FIXED_NOW,
                )
                ...
                self.assertEqual(len(review), 1)
                self.assertEqual(
                    review[0]["reason"], "offer_requires_manual_confirmation"
                )
