...
from analytics.feedback import feedback_id
from analytics.model import FEEDBACK_COLUMNS, write_csv_atomic
from analytics.rules import (
    RuleContext, _match_command, build_rules, main, select_rules, validate_rules,
)
...
class FeedbackRuleGenerationTests(unittest.TestCase):
    ...
    def test_rule_validation_recomputes_identity_and_rejects_malformed_fields(self):
        valid_id = "fb-" + "a" * 64
        rule = build_rules([event(feedback_id=valid_id)])[0]
        validate_rules([rule])
        mutations = {
            "rule_id": "rule-" + "0" * 64,
            "category": "unknown",
            ...
            "evidence_count": 2,
            "source_feedback_ids": [valid_id, valid_id],
            "last_updated": "not-a-date",
        }
        for field, value in mutations.items():
            with self.subTest(field=field):
                with self.assertRaises(ValueError):
                    validate_rules([{**rule, field: value}])
...
class FeedbackRuleCliTests(unittest.TestCase):
    ...
    def test_match_cli_emits_deterministic_json(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            source_ref = "a" * 64
            feedback_event = event(source_ref=source_ref)
            feedback_event["feedback_id"] = feedback_id(
                feedback_event["application_id"],
                feedback_event["occurred_at"],
                feedback_event["category"],
                source_ref,
            )
            rules = build_rules([feedback_event])
            rules_path.write_text(json.dumps(rules), encoding="utf-8")
            write_csv_atomic(
                rules_path.with_name("application_feedback.csv"),
                FEEDBACK_COLUMNS,
                [feedback_event],
            )
            argv = [
            ...
            drifted = [{**rules[0], "trigger": "fabricated drift"}]
            rules_path.write_text(json.dumps(drifted), encoding="utf-8")
            context = RuleContext("applied_ai", "senior", "US", "application", "unknown")
            with self.assertRaisesRegex(ValueError, "canonical derivation"):
                _match_command(rules_path, context)
            rules_path.with_name("application_feedback.csv").unlink()
