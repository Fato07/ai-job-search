...
class DashboardBuildTests(unittest.TestCase):
    ...
    def _write_cli_fixture(self, root):
        ...
        return paths
    def test_load_inputs_rejects_malformed_feedback_and_rules(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            paths = self._write_cli_fixture(root)
            paths.feedback.write_text("bad,header\n1,2\n", encoding="utf-8")
        ...
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            paths = self._write_cli_fixture(root)
            paths.rules.write_text('[{"rule_id":"rule-bad"}]\n', encoding="utf-8")
            with self.assertRaises(ValueError):
                _load_inputs(root)
    ...
    def test_utc_reporting_timezone_controls_operational_day_and_week_boundaries(self):
        events = [
            event("s-1", "app-1", "submitted", "2026-08-24T00:30:00Z"),
        ]
        ...
        self.assertEqual(snapshot["meta"]["reporting_timezone"], "UTC")
            ...
            class ProfileThenUnavailable:
                ...
                def execute(self, slug, data):
                    ...
                    if slug == "GMAIL_GET_PROFILE":
                        return {"data": {"emailAddress": "mailbox@example.test"}}
    ...
    def test_sync_gmail_cli_keeps_timeout_and_mailbox_mismatch_fail_closed(self):
        ...
        for failure, message in (
            ...
            with self.subTest(failure=failure), tempfile.TemporaryDirectory() as tmp:
                root = Path(tmp)
                self._write_cli_fixture(root)
                with patch(
                    "analytics.refresh.ComposioClient",
                    return_value=FailingClient(failure),
                ), patch("dashboard.build.Path.cwd", return_value=root):
