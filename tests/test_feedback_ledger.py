...
class InceptionFeedbackTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        rejected = {column: "" for column in TRACKER_COLUMNS}
        rejected.update(
            ...
            notes="Generic closer-match rejection.",
        )
        active = {
            **rejected,
            ...
            "stage": "submitted",
            "status": "applied",
            "status_updated_at": "2026-08-02",
        }
        cls.applications = [rejected, active]
        cls.events = seed_inception_feedback(cls.applications, NOW)

    def test_only_rejected_application_gets_generic_feedback(self):
        self.assertEqual(len(self.events), 1)
        event = self.events[0]
        self.assertEqual(event["application_id"], "app-rejected")
        self.assertEqual(event["evidence_tier"], "boilerplate")
        self.assertEqual(event["category"], "competition_no_specific_signal")
        self.assertEqual(event["rule_effect"], "monitor")
        self.assertEqual(event["required_action"], "")
    ...
    def test_seed_output_obeys_schema_privacy_and_uniqueness(self):
        ...
        for event in self.events:
            ...
            self.assertNotRegex(
                event["evidence_excerpt"], r"[\w.+-]+@[\w.-]+\.[A-Za-z]{2,}"
            )
    ...
    def test_backfill_is_idempotent_across_insertion_times(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            output = root / "application_feedback.csv"
            write_csv_atomic(tracker, TRACKER_COLUMNS, self.applications)
            write_csv_atomic(output, FEEDBACK_COLUMNS, self.events)
            with redirect_stdout(StringIO()):
                _backfill_command(tracker, output)
            self.assertEqual(
