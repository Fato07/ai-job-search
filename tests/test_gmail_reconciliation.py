...
class RecordingClient:
    def __init__(self, address, account="candidate-mail"):
...
class GmailReconciliationTests(unittest.TestCase):
    ...
    def test_client_executes_composio_with_argv_and_no_shell(self, run):
        ...
        result = ComposioClient("candidate-mail").execute("GMAIL_GET_PROFILE", {"user_id": "me"})
        ...
        self.assertEqual(
            ...
            [
                ...
                "--account", "candidate-mail", "-d", '{"user_id":"me"}',
            ],
        )
    ...
    def test_client_command_failure_uses_payload_free_diagnostic(self, run):
        ...
        with self.assertRaisesRegex(ComposioError, "Composio command failed") as raised:
            ComposioClient("candidate-mail").execute("GMAIL_GET_PROFILE", {"user_id": "me"})
        self.assertNotIn("do-not-leak", str(raised.exception))
    ...
    def test_disconnected_or_expired_auth_is_an_expected_availability_error(self, run):
        for stderr in (
            ...
            with self.subTest(stderr=stderr):
                ...
                with self.assertRaises(ComposioUnavailableError):
                    ComposioClient("candidate-mail").execute(
    ...
    def test_client_rejects_empty_account_alias(self):
        with self.assertRaisesRegex(ValueError, "account alias"):
            ComposioClient(account=" ")

    def test_mailbox_profile_exact_case_insensitive_match_passes(self):
        client = RecordingClient("MAILBOX@EXAMPLE.TEST")
        verify_mailbox(
            client, "mailbox@example.test", expected_account_alias="candidate-mail"
        )
    ...
    def test_mailbox_mismatch_fails_before_any_message_read(self):
        ...
        with self.assertRaisesRegex(ComposioError, "mailbox mismatch"):
            verify_mailbox(
                ...
                expected_account_alias="candidate-mail",
            )
        self.assertEqual(client.calls, [("GMAIL_GET_PROFILE", {"user_id": "me"})])

    def test_mailbox_gate_rejects_wrong_account_alias_before_execute(self):
        client = RecordingClient("mailbox@example.test", account="other")
    ...
    def test_all_offer_like_language_requires_manual_review(self):
        base = {
            "subject": "Application update",
            "sender": "Example Co Talent",
            "messageTimestamp": "2026-08-24T10:00:00Z",
            "company": "Example Co",
            "role": "Applied AI Engineer",
        }
        offer_like = (
            "We are delighted to extend you an offer for the Applied AI Engineer role.",
            "We would like to discuss a potential job offer for the role.",
            "We previously decided not to proceed, but are pleased to extend you an offer.",
            "This is not a formal offer letter.",
            "We would like to offer you a chance to interview for the role.",
            "We would like to offer you a senior conditional engineering role.",
            "No formal offer letter has been issued.",
            "Your formal offer letter is attached.",
            "We may send your offer letter after final approval.",
            "Following your interview, we are delighted to extend you an offer for the role.",
            "As you may recall, we are pleased to offer you the Applied AI Engineer role.",
            "Your formal offer letter is attached, and you may contact us with questions.",
            "We are pleased to offer you the Senior Engineer role, subject to final approval.",
            "We may confirm later, we are pleased to offer you the Senior Engineer role.",
            "We would like to offer the position to you.",
            "We may offer you the position after approval.",
            "We are unable to offer you the position.",
            "You have been offered the position.",
            "We are extending an offer for the position.",
            "We rejected the earlier application, but later offered you the role.",
            (
                "We offer reasonable accommodations and individual feedback. "
                "You have also been offered the position."
            ),
        )
        for index, text in enumerate(offer_like):
            with self.subTest(text=text):
                signal = classify_message({
                    **base,
                    "messageId": f"fixture-offer-review-{index}",
                    "messageText": text,
                })
                self.assertEqual(signal.event_type, "offer")
                self.assertTrue(signal.requires_manual_review)

        self.assertIsNone(classify_message({
            **base,
            "messageId": "fixture-interview-chance-only",
            "messageText": "This is a chance to interview with the team.",
        }))
    def test_non_job_offer_tokens_also_require_manual_review(self):
        base = {
            "sender": "Example Co Talent",
            "messageTimestamp": "2026-08-24T10:00:00Z",
            "company": "Example Co",
        ...
        cases = (
            (
                "Application update",
                "We decided not to proceed and are unable to offer individual feedback.",
            ),
            (
                "Application update",
                "We received your application. We offer reasonable accommodations.",
            ),
            (
                "Application update",
                "We invite you to an interview. We offer reasonable accommodations.",
            ),
            ("Offer of individual feedback", "We decided not to proceed."),
            ("We offer reasonable accommodations", "We received your application."),
            ("We offer help", "We invite you to an interview."),
            ("Application update", "We are offering support during the process."),
            ("Application update", "The company offers assistance."),
        )
        for index, (subject, body) in enumerate(cases):
            with self.subTest(subject=subject, body=body):
                signal = classify_message({
                    **base,
                    "subject": subject,
                    "messageId": f"fixture-conservative-offer-{index}",
                    "messageText": body,
                })
                self.assertEqual(signal.event_type, "offer")
                self.assertTrue(signal.requires_manual_review)
    def test_offer_after_display_subject_limit_still_requires_review(self):
        signal = classify_message({
            "subject": "x" * 220 + " job offer",
            "sender": "Example Co Talent",
            "messageTimestamp": "2026-08-24T10:00:00Z",
            "messageText": "We received your application.",
            "messageId": "fixture-long-subject-offer",
            "company": "Example Co",
            "role": "Applied AI Engineer",
        })
        self.assertEqual(signal.event_type, "offer")
        self.assertTrue(signal.requires_manual_review)
        self.assertLessEqual(len(signal.subject), 200)
        self.assertNotIn("offer", signal.subject)

    def test_named_rejections_are_classified_as_rejected_boilerplate(self):
        cases = (
            "example_robotics_rejection",
            "example_decision_rejection_us_role",
            "example_bank_security_rejection",
            "example_equity_rejection",
            "example_data_rejection",
            "example_mobility_rejection",
        )
        for case in cases:
    ...
    def test_role_specific_example_bank_rejection_matches_only_security_role(self):
        signal = classify_message(self.by_case["example_bank_security_rejection"])
        self.assertEqual(signal.company, "example bank group")
        self.assertEqual(signal.role, "Senior Security Engineer (Data Platform)")
        applications = [
            {"application_id": "ai", "company": "Example Bank", "role": "AI Platform Engineer", "discovered_at": "2026-08-16"},
            {"application_id": "security", "company": "Example Bank", "role": "Senior Security Engineer (Data Platform)", "discovered_at": "2026-08-16"},
        ]
        result = match_application(
            signal, applications, {"example bank group": "example bank"}
        )
    ...
    def test_example_decision_us_role_matches_unique_berlin_london_application(self):
        signal = classify_message(self.by_case["example_decision_rejection_us_role"])
        applications = [{
            "application_id": "example_decision-fde",
        ...
        self.assertEqual(result.application_id, "example_decision-fde")
    ...
    def test_same_company_multi_role_ambiguity_never_guesses(self):
        signal = classify_message(self.by_case["example_bank_ambiguous_same_company"])
        applications = [
            {"application_id": "ai", "company": "Example Bank", "role": "AI Platform Engineer", "discovered_at": "2026-08-16"},
            {"application_id": "security", "company": "Example Bank", "role": "Senior Security Engineer (Data Platform)", "discovered_at": "2026-08-16"},
        ]
    ...
    def test_message_before_application_never_matches(self):
        signal = classify_message(self.by_case["example_equity_rejection"])
        result = match_application(signal, [{
            "application_id": "later",
            "company": "Example Equity",
    ...
    def test_fixtures_are_sanitized_and_complete_for_task_matrix(self):
        required = {
            "example_robotics_rejection", "example_decision_rejection_us_role", "example_bank_security_rejection",
            "example_equity_rejection", "example_data_rejection", "example_mobility_rejection",
            "application_confirmation", "interview_invitation",
            "example_bank_ambiguous_same_company", "unrelated_newsletter",
        }
