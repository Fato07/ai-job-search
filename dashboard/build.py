...
_RESPONSE_EVENTS = frozenset(
    event_type
    for event_type in (
        "received", "rejected", "interview", "offer", "hired", "offer_declined"
    )
...
_DECISION_EVENTS = frozenset(
    event_type
    for event_type in (
        "rejected", "withdrawn", "offer", "hired", "no_response", "offer_declined"
    )
    if event_type in LIFECYCLE_EVENT_TYPES
)
...
def _reporting_zone(config: Mapping[str, object]) -> tuple[str, ZoneInfo]:
    name = _text(config.get("reporting_timezone")) or "UTC"
    try:
...
def _event_sets(
    ...
    return {
        ...
        "offered": _ids_with_event(events, {"offer", "hired", "offer_declined"}),
    }
...
def _chronological_outcome_times(
    ...
    for application_id, application_events in grouped_events.items():
        ...
        for row, occurred_at in application_events:
            ...
            if event_type in {"offer", "hired", "offer_declined"}:
...
def _load_inputs(root: Path) -> tuple[
    ...
    from analytics.feedback import validate_feedback
    from analytics.rules import validate_rules

    analytics = root / "analytics"
    applications = read_tracker_rows(root / "job_search_tracker.csv")
    application_ids = {row["application_id"] for row in applications}
    feedback = read_csv_rows(analytics / "application_feedback.csv", FEEDBACK_COLUMNS)
    for item in feedback:
        validate_feedback(item, application_ids)
    rules = _load_json(analytics / "feedback_rules.json", list)
    ...
    return (
        ...
        load_local_config(analytics / "config.json"),
    )
