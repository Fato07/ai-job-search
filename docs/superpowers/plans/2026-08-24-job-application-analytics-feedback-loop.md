...
- Use only the Composio Gmail account alias `job-search`; verify it resolves to `fathindos.fd@gmail.com` before message reads.
...
```json
{
  ...
  "gmail_expected_address": "fathindos.fd@gmail.com",
...
- explicit scoped logistics events for CuspAI and Zapier;
- explicit role-alignment events for Dust and Lobby;
- explicit `ml_genai_evaluation` feedback for Nordea;
- observed `metric_rigor_provenance` feedback for Nordea;
- observed `leadership_people_evidence` and `communication_decision_clarity` feedback for Wise;
- observed `technical_depth` and `communication_decision_clarity` feedback for Dragonfly;
- observed logistics/B2B follow-up feedback for Digital Workforce;
- inferred feedback only when the tracker contains a concrete, labeled inference; and
- boilerplate-only events for RobCo, Carta, Databricks, Taktile, Bolt, and other generic “closer match” decisions.
...
Expected: every rejected application ID is present in the feedback ledger; Nordea, Wise, and Dragonfly each have actionable non-boilerplate events; no excerpt exceeds 280 characters.
...
- RobCo rejection;
- Taktile rejection whose email role says US while the tracker says Berlin/London;
- Luminor with two applications and one role-specific rejection;
- Carta, Databricks, and Bolt rejections;
...
`verify_mailbox` executes `GMAIL_GET_PROFILE` with `user_id=me` and requires exact case-insensitive equality with `fathindos.fd@gmail.com`.
...
Expected: tests pass; live dry-run confirms `fathindos.fd@gmail.com`, reports scan/match/review counts, and writes nothing.
...
- mailbox confirmed as `fathindos.fd@gmail.com`;
