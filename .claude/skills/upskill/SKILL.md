...
### Aggregate mode
1. Read `job_search_tracker.csv` only with this exact canonical header:
   ...
   If the file is legacy, stop and tell the user to run `python3 -m analytics.migrate job_search_tracker.csv --apply`; never patch or partially parse the header.
2. For each normalized row, note the stable `application_id`, `role`, `company`, and `fit_score`. `fit_score` is a 0–100 number where 100 = perfect fit. Lower scores expose more gaps.
...
3. **One weight per job**, both 0–100 on the same scale: `(100 - fit_score) / 100` for tracker rows, `(100 - rank_score) / 100` for ranked-only rows. If a job is in both (Step 3.1 matched it), prefer the tracker's numeric `fit_score` for the weight. A **blank or non-numeric `fit_score`** contributes no weight: fall back to a matched ranked entry's `rank_score`, otherwise skip the row from weighted scoring and report the count once. **Never treat a blank as 0** because that assigns maximum weight to unknown fit.
